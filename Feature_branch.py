...
def batch_gd(model, criterion, optimizer, train_loader, test_loader, epochs, device, args):
  ...
  for it in range(epochs):
    ...
    if test_acc > best_test_acc:
      if (args.rank==0):
        torch.save(model.module.conv_1.state_dict(), os.path.join(weight_save_root, "haf_vgg16_conv_1_dim-%d.pth" % args.branch_1_dim))
        torch.save(model.module.conv_m.state_dict(), os.path.join(weight_save_root, "haf_vgg16_conv_m_dim-%d.pth" % args.branch_m_dim))
        torch.save(model.module.conv_h.state_dict(), os.path.join(weight_save_root, "haf_vgg16_conv_h_dim-%d.pth" % args.branch_h_dim))
        print("model weights are saved to haf_vgg16_conv_1_dim-%d.pth, haf_vgg16_conv_m_dim-%d.pth, haf_vgg16_conv_h_dim-%d.pth" % (args.branch_1_dim, args.branch_m_dim, args.branch_h_dim) )
...
def main_worker(args):
    ...
    haf = HAF(lower_branch,middle_branch,higher_branch,args)

    device = torch.device("cuda:%d" % args.gpu if torch.cuda.is_available() else "cpu")
    # print(device)
    # haf.to(device)
    haf.cuda(args.gpu)
    haf = DistributedDataParallel(haf, device_ids=[args.gpu], output_device=args.gpu, find_unused_parameters=True)
